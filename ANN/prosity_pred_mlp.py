...
class MLP:
    ...
    def mse_loss(self, y_true, y_pred):
        ...
        loss = loss+ 10*np.mean((y_true - y_pred) ** 2) #+np.mean((zz_model-self.rpt_por_norm)**2)
        loss+=10*np.mean((self.arctan_rpt_norm.ravel()-arctan_model.ravel())**2)+10000*np.mean((grad_model.ravel() - self.grad_amp_por_norm.ravel())**2)
    ...
    def fit(self, X, y,rpt_por,rpt_zp,rpt_vpvs,xx,yy, learning_rate=0.01, num_epochs=1, verbose=True):
        ...
        for i in range(X.shape[1]):
            self.X_norm[:,i]=X[:,i]/np.linalg.norm(X[:,i])
        ...
        self.xx_norm = xx /np.linalg.norm(X[:,0])
    ...
    def predict(self, X):
        for i in range(self.X.shape[1]):
            X[:,i]=X[:,i]/np.linalg.norm(self.X.iloc[:,i])
...
mlp=MLP(2,(5,5),1,random_state=1)


# mlp.forward(X_train)
# xopt=scipy.optimize.fmin_bfgs(mlp.forward(X_train))
...
# # ax=plt.axes(projection='3d')
# # ax.plot_surface(xx,yy,zz,cmap='rainbow')
# grad_por=mlp.predict(np.column_stack((xx.ravel(),yy.ravel())))[:,1].reshape(xx.shape)
# t=griddata((Zp_rpt,VpVs_rpt),Por_rpt.ravel(),(xx,yy), method='linear')
# gradx = np.gradient(t, axis=0)
# grady = np.gradient(t, axis=1)
# grad_por_rpt=np.arctan(grady/gradx)
# grad_por_rpt[np.isnan(grad_por_rpt)]=0
# f1=plt.scatter(xx,yy,c=grad_por_rpt,marker='s',cmap='rainbow',vmin=grad_por.min(),vmax=grad_por.max())
# grad_model=mlp.predict(np.column_stack((Zp_rpt,VpVs_rpt)))[:,1]
# for i in range(Sw_rpt.shape[0]):
#     # plt.plot(Zp[:,i],VpVs[:,i],'b-o',linewidth = '0.5',markersize=3)
#         if i%20==0:
#             # plt.plot(Zp_rpt.reshape(Por_rpt.shape)[i,:], VpVs_rpt.reshape(Por_rpt.shape)[i,:], 'b-', linewidth='0.5', markersize=3)
#             f2=plt.scatter(Zp_rpt.reshape(Por_rpt.shape)[i,:],VpVs_rpt.reshape(Por_rpt.shape)[i,:],c=grad_model.reshape(Por_rpt.shape)[i,:],edgecolors='black',cmap='rainbow',vmin=grad_por.min(),vmax=grad_por.max())
#
#
# plt.colorbar(f1).set_label('grad')
# plt.xlabel('Zp')
# plt.ylabel('Vp Vs ratio')
# plt.figure()
#
# h1=plt.scatter(xx,yy,c=zz,marker='s',cmap='rainbow',vmin=zz.min(),vmax=zz.max())
# plt.colorbar(h1).set_label('Porosity')
# for i in range(Por_rpt.shape[1]):
#     plt.plot(Zp_rpt.reshape(Por_rpt.shape)[:,i],VpVs_rpt.reshape(Por_rpt.shape)[:,i],'b-o',linewidth = '0.5',markersize=3)
#     if i%2==0:
#         plt.plot(Zp_rpt.reshape(Por_rpt.shape)[:, i], VpVs_rpt.reshape(Por_rpt.shape)[:, i], 'b-', linewidth='0.5', markersize=3)
# h2=plt.scatter(df['Zp'],df['VpVs ratio'],c=df['Lithology'],cmap=cmap,norm=norm,s=15,marker='o', edgecolors='black', linewidths=0.5)
# plt.colorbar(h2).set_label('Lithology')
# plt.xlabel('Zp')
# plt.ylabel('Vp Vs ratio')
plt.show()
...
class MLP:
    ...
    def forward(self,X):
        ...
        for i in range(len(self.Ws)):
            ...
            self.as_.append(a)
        # self.W=W
        # self.as_[-1]=input_scaler.inverse_transform(self.as_[-1])
        self.as_[-1]=self.output_scaler.inverse_transform(self.as_[-1])
        return self.as_[-1]
    ...
    def mse_loss(self, y_true, y_pred):
        ...
        input_rpt = np.column_stack((self.xx.ravel(), self.yy.ravel()))
        ...
        loss = loss+ 1000*np.mean((y_true - y_pred) ** 2) +10*np.mean((zz_model-self.rpt_por)**2)
        loss+=10*np.mean((self.arctan_rpt.ravel()-arctan_model.ravel())**2)+100000*np.mean((grad_model.ravel() - self.grad_rpt.ravel())**2)
    ...
    def backward(self, X, y):
        ...
        result= scipy.optimize.minimize(obj_func, self.W,method='L-BFGS-B',options={'maxiter':1000})
    ...
    def fit(self, X, y,rpt_por,rpt_zp,rpt_vpvs,xx,yy, learning_rate=0.01, num_epochs=1, verbose=True):
        ...
        self.input_scaler.fit(X.to_numpy())
        ...
        self.arctan_rpt = np.arctan(self.grady_rpt / self.gradx_rpt)
        self.arctan_rpt[np.isnan(self.arctan_rpt)] = 0
        self.grad_rpt = np.sqrt(self.gradx_rpt ** 2 + self.grady_rpt ** 2)
        self.grad_rpt[np.isnan(self.grad_rpt)] = 0 
        self.rpt_por[np.isnan(self.rpt_por)]=0
...
h2=plt.scatter(Zp_rpt,VpVs_rpt,c=Por_rpt,edgecolors='black',cmap='rainbow',vmin=df['Porosity'].min(),vmax=df['Porosity'].max())
